...
Surprisingly, for rotational symmetries on fully equivariant data, [Wang et al. (2023)](#References) found that a G-CNN exhibits poorer performance compared to a RG-CNN. This is a surprising result because a G-CNN, i.e. a fully equivariant network, is designed to perform well on fully equivariant data. Possibly the training dynamics benefit from relaxing of the equivariance constraint. To investigate this, we use the framework described in [Park & Kim (2022)](#References) for measuring convexity and flatness using the Hessian spectra.
...
1. Relaxed steerable G-CNN outperforms steerable G-CNN (fully equivariant network) on fully rotationally equivariant data as shown in the experiment on the super resolution dataset in [Wang et al. (2023)](#References).
2. Relaxed G-CNN outperforms G-CNN on non-fully rotationally equivariant data as shown in the experiment on the smoke plume dataset in [Wang et al. (2022)](#References).
...
<div style="text-align: center;">
  ...
  <p>Annotated segmented image taken from [16]</p>
</div>
...
Whenever both $\rho_{in}$ and $\rho_{out}$ can be decomposed into smaller building blocks called **irreducible representations**, equivariance w.r.t. infinite group $G$ is achieved (see Appendix A.1 of [Bekkers et al., (2023)](#References)).
...
We use the following $3$ models from [Wang et al. (2023)](#References)'s experiment on the same dataset in [Results](#Results):
...
This is a synthetic $64 \times 64$ 2D smoke simulation dataset generated by PhiFlow [(Holl et al., 2020)](#References), where dispersion of smoke in a scene starting from an inflow position with a buoyant force is simulated (Figure 2). 
...
- Relaxed steerable G-CNN from [Wang et al. (2022)](#References) with relaxed equivariance w.r.t the C4 group. 
- Steerable G-CNN from [Weiler et al. (2019)](#References) with full equivariance w.r.t the C4 group.


### Training Dynamics Evaluation

To assess the training dynamics of a network, we are interested in the final performance and the generalizability of the learned parameters, which are quantified by the final RMSE, and the sharpness of the loss landscape near the final weight-point proposed in [Zhao et al., 2023](#References). 
...
This definition is an adaptation from the one in [Zhao et al. (2023)](#References). A sharper loss landscape around the model's final weights usually implies a greater generalization gap.
...
We reproduced and extended the relevant findings in [Wang et al. (2023)](#Reference) reaffirming the effectiveness of relaxed equivariant models and demonstrating that they are able to outperform fully equivariant models even on perfectly equivariant datasets. 
